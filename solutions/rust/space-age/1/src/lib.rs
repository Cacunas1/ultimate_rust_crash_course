// The code below is a stub. Just enough to satisfy the compiler.
// In order to pass the tests you can add-to or change any of this code.
const SECONDS_PER_DAY: u64 = 24 * 60 * 60;
const BASE_ORBITAL_PERIOD_IN_DAYS: f64 = 365.25;
const BASE_ORBITAL_PERIOD_IN_SECONDS: f64 = BASE_ORBITAL_PERIOD_IN_DAYS * (SECONDS_PER_DAY as f64);

#[derive(Debug)]
pub struct Duration {
    pub duration: u64,
}

impl From<u64> for Duration {
    fn from(s: u64) -> Self {
        Duration{duration: s}
    }
}

pub trait Planet {
    const ORBITAL_FACTOR: f64;

    fn years_during(d: &Duration) -> f64 {
        (d.duration as f64) / (Self::ORBITAL_FACTOR * BASE_ORBITAL_PERIOD_IN_SECONDS)
    }
}

pub struct Mercury;
pub struct Venus;
pub struct Earth;
pub struct Mars;
pub struct Jupiter;
pub struct Saturn;
pub struct Uranus;
pub struct Neptune;

impl Planet for Mercury {
    const ORBITAL_FACTOR: f64 = 0.2408467;
}
impl Planet for Venus {
    const ORBITAL_FACTOR: f64 = 0.61519726;
}
impl Planet for Earth {
    const ORBITAL_FACTOR: f64 = 1.0;
}
impl Planet for Mars {
    const ORBITAL_FACTOR: f64 = 1.8808158;
}
impl Planet for Jupiter {
    const ORBITAL_FACTOR: f64 = 11.862615;
}
impl Planet for Saturn {
    const ORBITAL_FACTOR: f64 = 29.447498;
}
impl Planet for Uranus {
    const ORBITAL_FACTOR: f64 = 84.016846;
}
impl Planet for Neptune {
    const ORBITAL_FACTOR: f64 = 164.79132;
}
